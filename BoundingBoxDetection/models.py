...
class YOLOLayer(nn.Module):
    ...
    def my_loss(self,pred,target):
        ...
        #loss = torch.mean(torch.abs(torch.sin(np.pi + abs_diff)))
        degree = torch.remainder((pred*180/np.pi) + 180,180)
        pred = degree*np.pi/180
...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path, img_size=416):
        ...
        # self.conv2d = nn.Conv2d(96,256,kernel_size=3,padding=1)
        self.up = nn.Upsample(size=(64,64))

    def forward(self, x, targets=None):

        img_dim = 800
        input_channels = x.shape[1]
        x = self.up(x)    #-----> upsample from [_,96,16,19] to [_96,64,64]
        loss = 0
