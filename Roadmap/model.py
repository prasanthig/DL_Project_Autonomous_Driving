...
class DecoderModel(nn.Module):
    def __init__(self,input):
        ...
        self.decoder = nn.Sequential(
            nn.Conv2d(self.input_channel,16,kernel_size = 3, padding=1), #2^4
            nn.ReLU(),
            nn.BatchNorm2d(16),
            nn.Upsample(scale_factor = 2),

            nn.Conv2d(16,32,kernel_size = 3, padding=1), #2^5
            nn.ReLU(),
            nn.BatchNorm2d(32),
            nn.Upsample(scale_factor = 2),

            nn.Conv2d(32,64,kernel_size = 3, padding=1), #2^6
            nn.ReLU(),
            nn.BatchNorm2d(64),
            nn.Upsample(scale_factor = 2),

            nn.Conv2d(64,128,kernel_size = 3, padding=1), #2^7
            nn.ReLU(),
            nn.BatchNorm2d(128),
            nn.Upsample(scale_factor = 2),

            nn.Conv2d(128,128,kernel_size = 3, padding=1), #2^8
            nn.ReLU(),
            nn.BatchNorm2d(128),
            nn.Upsample(scale_factor = 2)
        )
        self.conv = nn.Conv2d(128,1,kernel_size=3,padding=1) #Reduce to 1 channel
        
    def forward(self,x):
        output = self.decoder(x)
        output = nn.Upsample(size=(800,800))(output) #Upsample to 800 x 800
